...
function SignInPage() {
  ...
  useEffect(() => {
    fetch('/api/status').then(r => r.json()).then(data => {
      ...
      if (s.alipay_oauth && s.alipay_app_id) {
        list.push({
          id: 'alipay', name: 'Alipay', enabled: true,
          loginUrl: `${sv}/api/oauth/alipay?state=`,
        })
      }
      setProviders(list)
    }).catch(() => {})
  ...
  if (provider === 'alipay') {
    return (
      <svg className="w-4 h-4 text-blue-500" viewBox="0 0 24 24" fill="currentColor">
        <path d="M21.422 15.358c-3.22-1.386-6.847-2.699-10.992-3.808l-.208-.078c2.67-2.592 5.05-5.656 5.544-8.654H9.818l-.08.008V.633h8.764V0H6.944v2.637H3.602v1.6h3.342v1.84H4.461v1.6h3.288c-.233 2.07-1.287 4.162-2.926 5.994-1.517-1.713-2.755-3.68-3.305-5.388H0c.702 2.525 2.278 5.189 4.448 7.285-1.812 1.473-4.09 2.44-4.09 2.44v1.159s3.954-1.12 6.262-2.654c2.033 1.535 4.52 2.537 7.415 2.978l-.515 2.303h-5.582v1.6H6.944v1.734h3.342v5.54h1.6v-5.54h5.642l1.68 5.54h1.9l-1.68-5.54H21v-1.6h-3.48zM9.898 5.292h.242c1.688.375 3.074 2.234 3.625 4.34-2.349-.926-4.505-2.263-3.867-4.34z"/>
      </svg>
    )
  }
  return null
