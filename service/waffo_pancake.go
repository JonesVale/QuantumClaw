...
func CreateWaffoPancakeCheckoutSession(ctx context.Context, params *WaffoPancakeCreateSessionParams) (*WaffoPancakeCheckoutSession, error) {
	...
	privateKey, err := normalizeRSAPrivateKey(system_setting.GetFetchSetting().WaffoPancakePrivateKey)
	if err != nil {
		return nil, err
	}
	...
	req.Header.Set("X-Merchant-Id", system_setting.GetFetchSetting().WaffoPancakeMerchantID)
	req.Header.Set("X-Timestamp", timestamp)
	req.Header.Set("X-Signature", signature)
	if system_setting.GetFetchSetting().WaffoPancakeSandbox {
...
func ResolveWaffoPancakeTradeNo(event *waffoPancakeWebhookEvent) (string, error) {
	...
	if tradeNo := strings.TrimSpace(event.Data.OrderID); tradeNo != "" {
		topUp, _ := model.GetTopUpByTradeNo(tradeNo)
...
func resolveWaffoPancakeWebhookPublicKey(environment string) string {
	if environment == "prod" {
		return strings.TrimSpace(system_setting.GetFetchSetting().WaffoPancakeWebhookPublicKey)
	}
	return strings.TrimSpace(system_setting.GetFetchSetting().WaffoPancakeWebhookTestKey)
}
