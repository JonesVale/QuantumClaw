...
var (
	...
	orderLocks   = make(map[string]*orderLockEntry)
)
...
// LockSubscriptionOrder 锁定订阅订单（防止并发回调重复处理）
func LockSubscriptionOrder(tradeNo string) func() {
	orderLocksMu.Lock()
	entry, ok := orderLocks[tradeNo]
	if !ok {
		entry = &orderLockEntry{}
		orderLocks[tradeNo] = entry
	}
	entry.ref++
	orderLocksMu.Unlock()
...
// NormalizeResetPeriod 标准化额度重置周期值
...
// GetSubscriptionPlanById 根据ID查询订阅套餐
...
// CountUserSubscriptionsByPlan 统计用户购买某套餐的次数
...
func getUserGroupByIdTx(tx *gorm.DB, userId int) (string, error) {
	...
	var group string
	if err := tx.Model(&User{}).Where("id = ?", userId).Select("group").Scan(&group).Error; err != nil {
		return "", err
	}
	return group, nil
}
...
// CreateUserSubscriptionFromPlanTx 从套餐创建用户订阅（事务内）
...
func CompleteSubscriptionOrder(tradeNo string, providerPayload string, expectedPaymentProvider string, actualPaymentMethod string) error {
	...
	// 防止并发重入
	unlock := LockSubscriptionOrder(tradeNo)
	defer unlock()
	...
	if logUserId > 0 {
		...
		RecordLog(context.Background(), logUserId, LogTypeTopup, msg)
	}
	return nil
}
...
// GetAllActiveUserSubscriptions 获取用户所有活跃订阅
...
// HasActiveUserSubscription 检查用户是否有活跃订阅
...
// GetAllUserSubscriptions 获取用户所有订阅（含过期）
...
// AdminInvalidateUserSubscription 立即取消用户订阅
...
// AdminDeleteUserSubscription 硬删除用户订阅
...
// ExpireDueSubscriptions 批量过期到期订阅
...
// SubscriptionPreConsumeRecord 预消费记录（幂等保护）
...
// SubscriptionPreConsumeResult 预消费结果
...
// PreConsumeUserSubscription 预消费订阅额度
...
// RefundSubscriptionPreConsume 退还预消费额度（幂等）
...
// PostConsumeUserSubscriptionDelta 更新订阅已消费量（delta 为正消费/负退款）
...
// CleanupSubscriptionPreConsumeRecords 清理过期幂等记录
...
// ResetDueSubscriptions 重置到期的额度重置周期
